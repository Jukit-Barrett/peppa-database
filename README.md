...
Peppa Database 是一款使用PHP开发的数据库操作组件，此组件并无任何其他依赖，开箱即用。

如果你需要开发自己的框架，可以考虑集成之。如果遇到任何问题，请联系 Email: kitgor.architect@gmail.com 。

Github 地址: https://github.com/Kit-Mrz/simple-database
...
注意，如果您需要对 Peppa Database 进行扩展，在开发过程中可以安装下列工具，在投入生产是不需要使用的。
...
└── Unit
    ├── ConnectTrait.php 
    ├── PeppaDatabaseTest.php # 单元测试类
    ├── PeppaModelTest.php # 单元测试类
    └── PeppaTest.php # 单元测试类
```
...
class PeppaTest extends TestCase
{
    use ConnectTrait;
...
```sh
$ ./vendor/phpunit/phpunit/phpunit --configuration phpunit.xml ./test/Unit --filter PeppaTest::test1
```
...
```php
use Mrzkit\PeppaDatabase\Capsule\PluginManagerAbstract;
